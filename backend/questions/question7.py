...
def generate_random_financial_statement1(company_name, statement_type):
    ...
    if statement_type == "Income Statement":
        ...
        data["Cost of Goods Sold"] = round(data["Revenue"] * random.uniform(0.4, 0.7), 2)
        data["Gross Profit"] = round(data["Revenue"] - data["Cost of Goods Sold"], 2)
        data["Operating Expenses"] = round(data["Gross Profit"] * random.uniform(0.1, 0.3), 2)
        data["Operating Income"] = round(data["Gross Profit"] - data["Operating Expenses"], 2)
        data["Interest Expense"] = round(data["Operating Income"] * random.uniform(0.01, 0.05), 2)
        data["Income Before Tax"] = round(data["Operating Income"] - data["Interest Expense"], 2)
        data["Income Tax"] = round(data["Income Before Tax"] * random.uniform(0.2, 0.3), 2)
        ...
        data["Current Liabilities"] = round(data["Liabilities"] * random.uniform(0.4, 0.8), 2)
        ...
        data["Depreciation & Amortization"] = round(data["Net Income"] * random.uniform(0.1, 0.3), 2)
        data["Change in Working Capital"] = round(data["Net Income"] * random.uniform(-0.2, 0.2), 2)
        data["Cash from Operations"] = round(data["Net Income"]+ data["Depreciation & Amortization"] + data["Change in Working Capital"],2,)
        data["Capital Expenditures"] = round(-data["Cash from Operations"] * random.uniform(0.2, 0.5), 2)
